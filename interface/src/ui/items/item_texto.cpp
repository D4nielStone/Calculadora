...
Interface::Items::Texto::Texto(std::string* label_shared) : resolucao(12), label_shared(label_shared), texto(""), letra_padding({ 4, 4 })
{
...
Interface::Items::Texto::Texto(const std::string &l) : resolucao(12), frase(l), letra_padding({5, 4})
{
    configurar(); // Configura o item com as configurações padrão
}

// Método para atualizar o estado do Texto, incluindo a lógica de interação
void Interface::Items::Texto::atualizar()
{
...
void Interface::Items::Texto::renderizar() const
{
...
void Interface::Items::Texto::renderizar_texto()
{
    ...
    for (char& c : frase) {
        ...
        w_letter = ch.Size.x;
        h_letter = ch.Size.y;
        y_letter = static_cast<int>(box_pos.y + 10 - ch.Bearing.y + letra_padding.y);
        ...
        letras_rect.push_back({ paraNDC(char_rect, contexto), ch.TextureID });
    ...
    box_size.y = static_cast<int>(line_pos.y + 12 + letra_padding.y * 2);  // Altura do texto mais padding
...
void Interface::Items::Texto::configurar(const std::string &font_path, uint resolucao)
{
    carregarFonte(font_path); // Carrega a fonte especificada
}

// Método para definir a moldura do Texto
void Interface::Items::Texto::defMoldura(Formas::Moldura* m)
{
    pai = m;
    contexto = m->obtCtx();
}

void Interface::Items::Texto::definirEscondido(bool boleano)
{
    escondido = boleano;
}

void Interface::Items::Texto::quebrarLinha(bool boleano)
{
    vquebrarLinha = boleano;
}

// Destrutor do Texto que libera a memória alocada
Interface::Items::Texto::~Texto() = default;