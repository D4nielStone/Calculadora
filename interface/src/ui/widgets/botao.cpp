...
Interface::Widgets::Botao::Botao(const std::string& label_shared, std::function<void()>funcao_click, bool completo)
    : completo(completo)
...
Interface::Widgets::Botao::Botao(const std::string& label_shared, bool* callback, bool completo, bool quebrar_linha, float d_h)
...
void Interface::Widgets::Botao::atualizar()
{
    if (icon)
        icon->atualizar();
...
void Interface::Widgets::Botao::renderizar() const
{
    moldura.renderizar();
    Texto::renderizar();
