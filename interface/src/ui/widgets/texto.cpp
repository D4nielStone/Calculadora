...
Texto::Texto(std::string* label_shared) : label_shared(label_shared)
{
    configurar();
}

Texto::Texto(const std::string& l)
{
    configurar();
}
...
void Texto::renderizar_texto(const std::string &frase)
{
    ...
    if (quebrarLinha)
        ...
        painel->posicaoWidget.x = (box_pos.x + painel->obterRetangulo().w/DivisaoHorizontal) - painel->widgetPadding.x*2;
}

// Configura o texto com a fonte e resolução especificada
void Texto::configurar(const uint resolucao, const std::string &font_path)
{
