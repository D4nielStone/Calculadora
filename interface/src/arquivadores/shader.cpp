...
std::vector<std::pair<std::pair<const char*, const char*>, uint>> shaders;
GLuint shader_atual;
...
void Shader::compilar(const char* vertexPath, const char* fragmentPath) {
    for (std::pair<std::pair<const char*, const char*>, uint> shader : shaders)
    {
...
bool Shader::checkCompileErrors(uint shader, const std::string& type) {
    GLint success;
    GLchar infoLog[1024];
    if (type != "PROGRAM") {
...
bool Shader::checkLinkErrors(uint program) {
    GLint success;
    GLchar infoLog[1024];
