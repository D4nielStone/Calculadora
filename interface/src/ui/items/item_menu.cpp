...
Interface::Items::ItemMenu::ItemMenu(std::string* label_shared) : resolucao(12), label_shared(label_shared), texto(""), letra_padding({ 4, 4 })
{
...
Interface::Items::ItemMenu::ItemMenu(const std::string &l) : resolucao(12), frase(l), letra_padding({5, 4})
{
    configurar(); // Configura o item com as configurações padrão
}

// Método para atualizar o estado do ItemMenu, incluindo a lógica de interação
void Interface::Items::ItemMenu::atualizar()
{
...
void Interface::Items::ItemMenu::renderizar() const
{
...
void Interface::Items::ItemMenu::renderizar_texto()
{
...
void Interface::Items::ItemMenu::configurar(const std::string &font_path, uint resolucao)
{
    carregarFonte(font_path); // Carrega a fonte especificada
}

// Método para definir a moldura do ItemMenu
void Interface::Items::ItemMenu::defMoldura(Formas::Moldura* m)
{
    pai = m;
    contexto = m->obtCtx();
...
void Interface::Items::ItemMenu::definirEscondido(bool boleano)
{
    escondido = boleano;
}

void Interface::Items::ItemMenu::quebrarLinha(bool boleano)
{
    vquebrarLinha = boleano;
}

// Destrutor do ItemMenu que libera a memória alocada
Interface::Items::ItemMenu::~ItemMenu() = default;